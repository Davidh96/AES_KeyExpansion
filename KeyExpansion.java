...
public class KeyExpansion {
    ...
    public static void main(String args[]) {
        ...
        int expKeySize = 44*4;
        byte[] w = new byte[expKeySize];

        String key1 = "0f1571c947d9e8590cb7add6af7f6798";

        //convert key to byte array
        key = new BigInteger(key1, 16).toByteArray();

        // Initialise first words with  key
        ...
        while (i < expKeySize) {
            ...
            //w[i]=w[i-4] xor temp
            for (int j = 0; j < 4; j++){
                w[i + j] = (byte) (w[i - keySize + j] ^ temp[j]);
            }
    ...
    //replace byte with sbox value
    private static void subWord(byte[] word) {
        for (int i = 0; i < 4; ++i){
            word[i] = (byte) s[word[i] & 0xff];
        }
